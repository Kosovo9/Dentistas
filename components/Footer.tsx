...
const Footer: React.FC = () => {
  const { t } = useLanguage();
  // Número solicitado: +52 1 614 327 7218
  const devWhatsappUrl = "https://wa.me/5216143277218?text=" + encodeURIComponent("¡Hola! Me encantó la App/Landing que hiciste para el Dr. Enrique Acosta. Me gustaría preguntar por una para mi proyecto sin compromiso.");

  return (
    <footer className="bg-navy-luxury text-white/50 py-20 border-t border-white/5 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        {/* CTA DEL DESARROLLADOR - ALTA CONVERSIÓN */}
        <motion.div 
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mb-20 p-1 bg-gradient-to-r from-gold-luxury/20 via-white/5 to-gold-luxury/20 rounded-3xl"
        >
          <div className="bg-navy-luxury/80 backdrop-blur-xl p-10 md:p-14 rounded-[calc(1.5rem-4px)] text-center group">
            <h4 className="text-white luxury-font text-2xl md:text-3xl mb-8 font-light italic leading-tight">
              {t('footer_dev_cta')}
            </h4>
            <a 
              href={devWhatsappUrl}
              ...
              className="inline-flex items-center gap-4 bg-white text-navy-luxury px-12 py-5 rounded-full font-black uppercase tracking-[0.2em] text-[10px] shadow-[0_20px_40px_rgba(255,255,255,0.1)] hover:bg-gold-luxury hover:text-navy-luxury hover:scale-105 transition-all duration-500 active:scale-95"
            >
